...
const AddEmr: React.FC = () => {
  ...
  const onSubmit = async (data: EMRInputs) => {
    ...
    // Convert string fields (comma separated) into arrays
    const emrData = {
      patientId,
      aadhar:0 , // temporary or optional if backend doesn’t need
      allergies: data.allergies
        ? data.allergies.split(",").map((s) => s.trim())
        : [],
      diseases: data.diseases
        ? data.diseases.split(",").map((s) => s.trim())
        : [],
    ...
    try {
      const res = await createEMR(emrData);
      console.log("✅ EMR Added Successfully:", res);
      alert("✅ EMR Added Successfully!");
      reset();
    } catch (error) {
      console.error("❌ Failed to add EMR:", error);
      alert("Failed to add EMR. Please check console for details.");
    }
  };
