              ...
              {bookings.map((booking, idx) => (
                <tr key={idx} className="border-t hover:bg-gray-50 text-xs sm:text-sm md:text-base">
                  <td className="px-3 sm:px-4 py-2 font-medium flex items-center gap-2">
                    ...
                    <span className="truncate">{booking.patient?.name || "-"}</span>
                  </td>
                  <td className="px-3 sm:px-4 py-2">{booking.patient?.gender || "-"}</td>
                  <td className="px-3 sm:px-4 py-2">{booking.patient?.age ? `${booking.patient.age} yrs` : "-"}</td>
                  <td className="px-3 sm:px-4 py-2">
                    <div className="flex items-center gap-2">
                      <Phone size={18} className="text-gray-500 flex-shrink-0" />
                      <span className="truncate">{booking.patient?.contact || "N/A"}</span>
                    </div>
                  </td>
                  <td className="px-3 sm:px-4 py-2">
