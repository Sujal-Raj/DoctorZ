...
//     const dates =
//       selectionType === "single"
//         ? selectedSingleDate ? [selectedSingleDate.toISOString()] : []
//         : selectedMultipleDates.map((d) => d.toISOString());
...
//       if (data.createdDates.length > 0) {
//         alert(`Slots created for: ${data.createdDates.map(formatDatePure).join(", ")}`);
//       }

//       if (data.alreadyExistDates.length > 0) {
//         alert(`Slots already exist for: ${data.alreadyExistDates.map(formatDatePure).join(", ")}`);
//       }
...
//       ? savedSlots.filter(
//           (s) => new Date(s.date).toDateString() === selectedSingleDate.toDateString()
//         )
//       : selectionType === "multiple" && selectedMultipleDates.length > 0
//       ? savedSlots.filter((s) =>
//           selectedMultipleDates.some((d) => new Date(s.date).toDateString() === d.toDateString())
//         )
//       : savedSlots;

//   const totalPages = Math.ceil(filteredSlots.length / slotsPerPage);
//   const paginatedSlots = filteredSlots.slice((currentPage - 1) * slotsPerPage, currentPage * slotsPerPage);
...
//         <p className="text-center text-gray-500 mt-6">No slots available for selected date(s)</p>
//       ) : (
//         paginatedSlots.map((slotItem) => (
//           <div key={slotItem._id} className="mt-3 bg-gray-50 border border-gray-200 rounded-xl p-4 shadow-sm">
//             <p className="mb-2 font-semibold text-gray-700">{new Date(slotItem.date).toDateString()}</p>
...
//                     key={s.time}
//                     className={`px-3 py-1 rounded-lg text-sm font-medium transition transform hover:scale-105 ${
//                       s.isActive ? "bg-green-500 text-white hover:bg-green-600" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
//                     }`}
...
//           <button
//             className={`px-4 py-2 rounded-lg border ${
//               currentPage === 1 ? "bg-gray-200 text-gray-500 cursor-not-allowed" : "bg-white text-gray-800 hover:bg-gray-100"
//             }`}
//             disabled={currentPage === 1}
//             onClick={() => setCurrentPage((prev) => prev - 1)}
//           >
//             &lt;
//           </button>
//           <span className="px-3 py-2 rounded-lg bg-gray-100 text-gray-700">{currentPage} / {totalPages}</span>
//           <button
//             className={`px-4 py-2 rounded-lg border ${
//               currentPage === totalPages ? "bg-gray-200 text-gray-500 cursor-not-allowed" : "bg-white text-gray-800 hover:bg-gray-100"
//             }`}
//             disabled={currentPage === totalPages}
//             onClick={() => setCurrentPage((prev) => prev + 1)}
//           >
